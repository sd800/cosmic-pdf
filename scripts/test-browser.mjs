...
try{
 ...
 await frame.locator('#search-toggle').click();assert.equal(await frame.locator('#browser-find-hint').isVisible(),true);assert.equal(await frame.locator('#query').isVisible(),false);
 ...
 assert.ok((await frame.locator('#properties-dialog').boundingBox()).width<=600);assert.equal(await frame.locator('#properties-list').evaluate(n=>getComputedStyle(n).fontSize),'15px');
...
assert.match(await frame.locator('[data-property=pageSize]').textContent(),/215.9 × 297.04 mm/);
 ...
 await menuAction('search-toggle');await frame.waitForSelector('#findbar:not([hidden])');assert.equal(await frame.locator('#browser-find-hint').isVisible(),true);await frame.locator('#find-close').click();await prefs.locator('#useChromeFind').uncheck();await frame.waitForFunction(()=>!qaSettings().useChromeFind);await menuAction('search-toggle');assert.equal(await frame.locator('#query').isVisible(),true);await prefs.locator('#useChromeFind').check();await frame.waitForFunction(()=>qaSettings().useChromeFind&&document.querySelector('#findbar').hidden);await menuAction('print');await frame.waitForSelector('#print-dialog[open]');await frame.locator('#print-dialog button[value=cancel]').click();await menuAction('properties');await frame.waitForSelector('#properties-dialog[open]');await frame.locator('#properties-close').click();
