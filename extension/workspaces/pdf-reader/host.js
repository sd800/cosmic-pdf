...
document.documentElement.lang=locale; $('intro').textContent=text.intro;$('open-file').textContent=text.open;$('settings').textContent=text.settings;$('native').textContent=text.native;$('hint').textContent=text.hint;$('loading').setAttribute('aria-label',text.loading);document.documentElement.dataset.motion=String(settings.motion);
...
function fail(message){$('message').textContent=message; $('loading').hidden=true; setView('home');}
...
async function load(loader){
 ...
 reader?.destroy();reader=null;activeSettings=settings;$('reader').hidden=false;setView('loading');$('loading').hidden=false;$('message').textContent='';
...
async function openFile(file){if(file.size>PDF_LIMITS.bytes){if(reader)reader.showError(text.large);else fail(text.large);return;}source=null;history.replaceState(null,'',chrome.runtime.getURL(READER_PATH));$('native').hidden=true;await load(async()=>({bytes:await file.arrayBuffer(),filename:file.name}));}
function loadSource(){return load(async signal=>{const response=await fetch(source,{signal,credentials:'include',cache:'default',referrerPolicy:'no-referrer'});return {bytes:await readResponse(response,signal),filename:filenameFrom(response.url||source,response.headers.get('content-disposition')||'')};});}
...
window.addEventListener('pageshow',event=>{if(!event.persisted)return;if(original){const saved=original,name=filename;void load(async()=>({bytes:await saved.arrayBuffer(),filename:name}));}else if(source)void loadSource();else setView('home');});
